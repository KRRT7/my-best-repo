from bubble_sort import find_last_node_typed


def test_happy_path():
    nodes = [{"id": 1}, {"id": 2}, {"id": 3}]
    edges = [{"source": 1, "target": 2}, {"source": 2, "target": 3}]
    assert find_last_node_typed(nodes, edges) == {"id": 3}


def test_no_last_node_cycle():
    nodes = [{"id": 1}, {"id": 2}]
    edges = [{"source": 1, "target": 2}, {"source": 2, "target": 1}]
    assert find_last_node_typed(nodes, edges) is None


def test_no_last_node_disconnected():
    nodes = [{"id": 1}, {"id": 2}, {"id": 3}, {"id": 4}]
    edges = [{"source": 1, "target": 2}, {"source": 3, "target": 4}]
    result = find_last_node_typed(nodes, edges)
...
def test_single_node():
    nodes = [{"id": 1}]
    edges = []
    assert find_last_node_typed(nodes, edges) == {"id": 1}
...
def test_node_with_no_outgoing_but_incoming():
    ...
    assert find_last_node_typed(nodes, edges) == {"id": 2}
...
def test_large_graph_with_last_node():
    ...
    assert find_last_node_typed(nodes, edges) == {"id": num_nodes}
...
def test_nodes_with_string_ids():
    ...
    assert find_last_node_typed(nodes, edges) == {"id": "c"}
