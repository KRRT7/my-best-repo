...
class GCodeProcessor:
    ...
    def calculate_distance(point1, point2):
        # Inline computation without function call overhead, adjusted logging level
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        points = []
        add_wobble = self.config.xy_thickness
        ...
        for i in range(num_points + 1):
            t = i / num_points
            # Base position
            x, y = start_point[0] + dx * t, start_point[1] + dy * t

            # Add wobble to all points except start and end
            if 0 < i < num_points and length > 0:
                wobble = random.uniform(-add_wobble, add_wobble)
                x += wobble * (-dy / length)
                y += wobble * (dx / length)
            ...
            # For all points except the first one, calculate the delta E
            if i > 0:
                e_delta = e - last_e
            else:
                e_delta = e
