...
class GCodeProcessor:
    ...
    def calculate_distance(point1, point2):
        # Pre-calculate differences to avoid repeated subtraction
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        max_dist_inv = 1 / self.config.xy_point_dist
        num_points = max(2, int(original_distance * max_dist_inv))
        
        points = []
        sx, sy, sz, _ = start_point
        ex, ey = end_point[:2]
        
        last_e = start_point[3] if len(start_point) > 3 else 0

        # Calculate directional components outside loop
        dx = ex - sx
        dy = ey - sy
        length = math.sqrt(dx * dx + dy * dy)
        
        if length > 0:
            # Pre-compute constants for wobble calculation
            inv_length = 1 / length
            dx_perp = -dy * inv_length
            dy_perp = dx * inv_length
        else:
            dx_perp = dy_perp = 0  # No wobble if no movement
        ...
        for i in range(num_points + 1):
            ...
            e = total_extrusion * t if len(start_point) <= 3 else start_point[3] + total_extrusion * t

            if i > 0:
                e_delta = e - last_e
                points.append((x, y, sz, e_delta))
            else:
                points.append((x, y, sz, e))
