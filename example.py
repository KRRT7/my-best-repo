...
class GCodeProcessor:
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        xy_thickness = self.config.xy_thickness
        start_x, start_y, start_z = start_point[0], start_point[1], start_point[2]

        def wobble_point(i, t):
            x = start_x + dx * original_distance * t
            y = start_y + dy * original_distance * t

            if 0 < i < num_points:
                wobble = random.uniform(-xy_thickness, xy_thickness)
                x += wobble * -dy
                y += wobble * dx

            e = (start_point[3] + total_extrusion * t) if len(start_point) > 3 else total_extrusion * t
            return x, y, start_z, e

        for i in range(num_points + 1):
            t = i / num_points
            x, y, z, e = wobble_point(i, t)
