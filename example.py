...
class GCodeProcessor:
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        dx = (end_point[0] - start_point[0]) / num_points
        dy = (end_point[1] - start_point[1]) / num_points
        dz = start_point[2]
        wobble_strength = self.config.xy_thickness

        points = []
        last_e = start_point[3] if len(start_point) > 3 else 0
        length = math.sqrt(dx * dx + dy * dy)
        norm_dx, norm_dy = (-dy / length, dx / length) if length > 0 else (0, 0)

        for i in range(num_points + 1):
            t = i / num_points
            # Base position
            x = start_point[0] + dx * i
            y = start_point[1] + dy * i

            # Add wobble to all points except start and end
            if 0 < i < num_points:
                wobble = random.uniform(-wobble_strength, wobble_strength)
                x += wobble * norm_dx
                y += wobble * norm_dy
