...
class GCodeProcessor:
    ...
    def calculate_distance(point1, point2):
        # Inline the distance calculation for direct return to avoid logging overhead in production.
        return math.sqrt(
            (point2[0] - point1[0]) ** 2 +
            (point2[1] - point1[1]) ** 2 +
            (point2[2] - point1[2]) ** 2
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        """Apply fuzz skin effect along the perimeter."""
        ...
        points = []
        len_start_point = len(start_point)
        last_e = start_point[3] if len_start_point > 3 else 0

        dx = end_point[0] - start_point[0]
        dy = end_point[1] - start_point[1]
        length = 1 / math.sqrt(dx * dx + dy * dy) if dx != 0 or dy != 0 else 0

        apply_wobble = self.config.xy_thickness
        x_base = start_point[0]
        y_base = start_point[1]
        ...
        for i in range(num_points + 1):
            ...
            if 0 < i < num_points:
                wobble = random.uniform(-apply_wobble, apply_wobble)
                x -= wobble * dy * length
                y += wobble * dx * length
