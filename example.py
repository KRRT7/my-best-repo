...
class GCodeProcessor:
    ...
    def interpolate_with_constant_resolution(
        ...
        if distance == 0:
            logging.debug(
                f"No interpolation needed for identical points: {start_point}"
            )
            return []
        ...
        for i in range(num_segments + 1):
            ...
            compensation_factor = (
                distance / segment_length if segment_length != 0 else 1
            )
            compensation_factor = (
                compensation_factor**self.config.bridge_compensation_multiplier
            )
            compensated_extrusion = (
                extrusion_per_segment * compensation_factor
                if self.config.compensate_extrusion
                else extrusion_per_segment
            )
    ...
    def detect_gcode_flavor(self, gcode_lines: List[str]) -> Optional[str]:
        # Cache the prefix to avoid re-evaluating it in every iteration
        prefix = "; gcode_flavor ="
        prefix_length = len(prefix)
        for line in gcode_lines:
            # Check prefix using slicing for slightly better performance
            if line[:prefix_length] == prefix:
                # Use split once, this slightly optimizes the operation
    ...
    def _process_basic_fuzzy_settings(
        ...
        """Optimized fuzzy settings processing logic"""
        fuzzy_enabled, point_dist, thickness, support_contact_dist = False, None, None, None

        # Using a single pass through the lines to optimize the processing of fuzzy skin settings
        for line in reversed(gcode_lines):
            if line.startswith(self.lookup["fuzzy_skin"]):
                ...
                if fuzzy_skin_value in self.lookup["fuzzy_skin_values"]:
                    fuzzy_enabled = True
                    logging.error(
                        "Paint-On Fuzzyskin requires to turn off normal fuzzyskin in the slicer."
                    )
                    sys.exit(1)
                # Break after finding the fuzzy_skin line if not enabled
                break
            ...
            if line.startswith(self.lookup["supportContact"]) and support_contact_dist is None:
                try:
                    support_contact_dist = float(line.split("=")[-1].strip())
                    logging.debug(f"Support contact distance: {support_contact_dist}")
                except ValueError:
                    logging.warning(
                        "Invalid value for support_material_contact_distance"
                    )
                break
    ...
    def process_movement_line(
        ...
        try:
            ...
            if "X" in coordinates or "Y" in coordinates:
                current_point = (
                    coordinates.get(
                        "X", self.previous_point[0] if self.previous_point else 0
                    ),
                    coordinates.get(
                        "Y", self.previous_point[1] if self.previous_point else 0
                    ),
    ...
    def process_file(self) -> None:
        ...
        for line in gcode_lines:
            if line.strip().startswith("M82"):
                logging.error(
                    "Absolute extrusion mode (M82) detected. This script only works with relative extrusion mode (M83)."
                )
                return
        ...
        fuzzy_enabled, point_dist, thickness, support_contact_dist = (
            self.process_fuzzy_skin_settings(gcode_lines)
        )

        # Apply G-code settings where command line args weren't specified
        self.config.apply_gcode_settings(
            fuzzy_enabled, point_dist, thickness, support_contact_dist
        )
    ...
    def process_line(self, line: str) -> List[str]:
        ...
        # Check for top solid infill
            ...
            line.startswith(self.lookup["top_solid_infill"])
            and self.in_fuzzy_section
            and self.config.top_surface
        ):
            self.in_external_perimeter = False
            ...
            if self.in_external_perimeter and self.in_fuzzy_section:
                if "E" not in line:  # This is the positioning move
                    ...
                    if result and result[0]:  # Check if we got a valid point
                        ...
                        logging.debug(
                            f"Setting initial perimeter position: {current_point}"
                        )
                        self.previous_point = current_point
                    return [line]
                return self.handle_external_perimeter_movement(line)
            elif self.in_top_solid_infill or (
                self.in_bridge and self.config.lower_surface
            ):
    ...
    def handle_external_perimeter_movement(self, line: str) -> List[str]:
        ...
        points = self.interpolate_with_constant_resolution_XY(
            self.previous_point, current_point, e_value
        )
    ...
    def parse_point(self, line: str) -> Optional[Tuple[float, float, float, float]]:
        ...
        try:
            ...
            if (
                coords["Z"] is None
                and hasattr(self, "previous_point")
                and self.previous_point
            ):
                ...
                for j in range(i + 1, tool_config_end):
                    if gcode_lines[j].strip() == "T1":
                        ...
                        for k in range(i + 1, tool_config_end):
                            gcode_lines[k] = ""
                        logging.debug(
                            f"Marked fuzzy section start at line {i} and cleared config block"
                        )
                        i = tool_config_end
                        break
                        ...
                        for k in range(i + 1, tool_config_end):
                            gcode_lines[k] = ""
                        logging.debug(
                            f"Marked fuzzy section end at line {i} and cleared config block"
                        )
                        i = tool_config_end
                        break
            else:
    ...
    def _mark_fuzzy_sections_bambu(self, gcode_lines: List[str]) -> List[str]:
        ...
        while i < len(gcode_lines):
            ...
            if current_line == ";FuzzyTool":
                ...
                if first_fuzzy_tool:
                    logging.debug(
                        f"Keeping first tool block from line {i} to {end_index}"
                    )
                    first_fuzzy_tool = False
                    i = end_index + 1
                    continue
                ...
                logging.debug(
                    f"Found {'Fuzzy' if 'Fuzzy' in current_line else 'NonFuzzy'}Filament at line {i}"
                )
                eos_marker = (
                    ";FuzzyFilamentEOS"
                    if current_line == ";FuzzyFilament"
                    else ";NonFuzzyFilamentEOS"
                )
                eos_index = i + 1
                found_eos = False
                ...
                if first_filament_section:
                    logging.debug(
                        f"Keeping first filament section from line {i} to {eos_index}"
                    )
                    ...
                    logging.debug(
                        f"Processing subsequent filament section from line {i} to {eos_index}"
                    )
                    if current_line == ";FuzzyFilament":
                        gcode_lines[i] = ";FuzzySectionStart\n"
                        ...
                        for j in range(i + 1, eos_index + 1):
                            logging.debug(
                                f"Clearing line {j}: {gcode_lines[j].strip()}"
                            )
                            gcode_lines[j] = ""
                    else:  # NonFuzzy
