...
class GCodeProcessor:
    ...
    def handle_top_solid_infill(self, line: str) -> List[str]:
        # Speed up: compile the regex pattern once and store it
        if not hasattr(self, '_speed_pattern'):
            self._speed_pattern = re.compile(r"F([-+]?[0-9]*\.?[0-9]+)")

        self.in_top_solid_infill = True
        self.previous_point = None
        result = [line]

        # Directly check config's fuzzy speed and append speed change, if applicable
        fuzzy_speed = self.config.fuzzy_speed
        if fuzzy_speed is not None:
            current_speed_match = self._speed_pattern.search(line)
            if current_speed_match:
                self.current_speed = float(current_speed_match.group(1))
            
            # Append updated speed command if fuzzy_speed is different from current
            if fuzzy_speed != self.current_speed:
                result.append(f"G1 F{fuzzy_speed}\n")
        return result
