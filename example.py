...
class GCodeProcessor:
    ...
    def calculate_distance(point1, point2):
        # Using inline calculations to avoid variable overhead
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        # Pre-allocate the list size for performance
        ...
        extra_start = start_point[3] if len(start_point) > 3 else 0

        dx = end_x - start_x
        dy = end_y - start_y
        segment_length = math.sqrt(dx * dx + dy * dy)

        # Combine conditional calculation to reduce branching
        if segment_length > 0:
            inv_segment_length = 1 / segment_length
            norm_dx = dx * inv_segment_length
            norm_dy = dy * inv_segment_length
        else:
            norm_dx = norm_dy = 0

        last_e = 0

        step_x = dx / num_points
        step_y = dy / num_points
        step_e = total_extrusion / num_points
        ...
        for i in range(num_points + 1):
            ...
            e = extra_start + step_e * i

            if i == 0:
                points[i] = (x, y, start_z, e)
            else:
                e_delta = e - last_e
                points[i] = (x, y, start_z, e_delta)
