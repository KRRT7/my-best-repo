...
class GCodeProcessor:
    ...
    def interpolate_with_constant_resolution_XY(
        ...
        # Use integer division and max(2, ...) to ensure at least two points are generated
        num_points = max(2, int(original_distance / self.config.xy_point_dist))

        # Precalculate values for the loop
        dx = end_point[0] - start_point[0]
        dy = end_point[1] - start_point[1]
        dz = start_point[2]  # Z coordinate remains constant
        length = math.sqrt(dx * dx + dy * dy)

        points = []
        last_e = start_point[3] if len(start_point) > 3 else 0
        
        # Precalculate the inverse of num_points for the loop
        inv_num_points = 1.0 / num_points
        ...
        for i in range(num_points + 1):
            ...
            if 0 < i < num_points and length > 0:
                wobble = random.uniform(-self.config.xy_thickness, self.config.xy_thickness)
                # Optimize computation of perpendicular wobble direction
                x += wobble * (-dy / length)
                y += wobble * (dx / length)
            ...
            # Store relative extrusion, e_delta
            e_delta = e - last_e
            points.append((x, y, dz, e_delta))
